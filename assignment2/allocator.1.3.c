...
static void MergeFreeList(byte *AddHeader, header_type *h);
...
static void MergeFreeList(byte *AddHeader, header_type *h) {
    int i = FindPositionInFilelist(AddHeader);
    if (i != -1) {
        // merge with previous free chunk
        byte *addprev = my_heap.free_list[i - 1];
        if (addprev != NULL) {
            header_type *prev = (header_type *)addprev;
            byte *addAfterPrev = (byte *)addprev + prev->size;
            if (addAfterPrev == AddHeader) {
                prev->size = h->size + prev->size;
                DeleteInFileist(AddHeader);
            }
        }
        // merge with next free chunk
        byte *addnext = (byte *)AddHeader + h->size;
        header_type *next = (header_type *)addnext;
        if (addnext != NULL && next->status == FREE) {
            h->size = h->size + next->size;
            DeleteInFileist(addnext);
        }
    }
}