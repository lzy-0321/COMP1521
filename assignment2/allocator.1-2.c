...
void my_free(void *ptr) {
    ...
    // avoid producing an uninitialized pointer
    // Merge two thingy which is between in freelist
    merge();
}
...
static void merge() {
    for (int i = 0; i < my_heap.n_free; i++) {
        byte *addcurr = my_heap.free_list[i];
        header_type *curr = (header_type *)addcurr;
        byte *addnext = my_heap.free_list[i] + curr->size;
        header_type *next = (header_type *)addnext;
        if (curr->status == FREE && next->status == FREE) {
            curr->size += next->size;
