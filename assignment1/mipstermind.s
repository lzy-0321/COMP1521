...
play_game__body:
	...
	li   $s0, 0          # turn = 0, t0 = turn

loop0:

	bge	 $s0, MAX_TURNS, pgbend0	# turn >= MAX_TURNS go next

	move $a0, $s0
	jal  play_turn		# int turn_status = play_turn(turn);
	move $t1, $v0
	beq	 $t1, TURN_WIN, pgbend	# if $v0 == turn_win then end
...
loop2:
	...
	sw	 $v0, ($t4)		# correct_solution[i] = rand(GUESS_CHOICES) + 1
...
play_turn__body:
	...
	beq	 $s1, GUESS_LEN, plnext0	# if $t0 == GUESS_LEN then nest
...
loop3:
	...
	sw	$t1, ($t4)		# current_guess[n_guess] = guess;
...
loop4:
	...
	lw	$t5, ($t3)		# t5 = current_guess[i]

	la  $t4, solution_temp  #
    add $t6, $t4, $t1

	sw  $t5, ($t6)		# solution_temp[i] = current_guess[i]
...
loop5:
	...
	lw	$t5, ($t4)		# int guess = current_guess[guess_index]

	la  $t3, solution_temp  #
    add $t6, $t3, $t2

	lw  $t7, ($t6)		# solution_temp[guess_index]
