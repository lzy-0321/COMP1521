...
loop0:
	...
	bge	$t0, MAX_TURNS, end0	# turn >= MAX_TURNS go next

	move $a0, $t0
	jal  play_turn		# int turn_status = play_turn(turn);

	beq	 $v0, TURN_WIN, end	# if $v0 == turn_win then end
...
generate_solution__body:
	...
	li   $t1, 0 				# int i = 0
loop2:
	bge	 $t1, GUESS_LEN, next2	# if $t0 >= GUESS_LEN then next0
	...
	sw	 $v0, ($t3)		# correct_solution[i] = rand(GUESS_CHOICES) + 1
